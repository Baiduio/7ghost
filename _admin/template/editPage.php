...
<?php include tpl('menu');?>
	<div class="main">
			...
			<input type="hidden" name='key' value='<?php _e($key)?>'></input>
				<h2 class="section-header">自定义页面<?php if(isset($item['name']) && $item['name']): ?>(<?php _e($item['name'])?>)<?php endif; ?> - 基本
				...
				<?php
					...
					w('select')->set('name','替换域名')
							...
							->set('value',isset($item['replaceDomain'])?$item['replaceDomain']:'0')
					...
					w('select')->set('name','替换HTML相对地址')
							...
							->set('value',isset($item['relativeHTML'])?$item['relativeHTML']:'0')
							->set('options',array('替换'=>'0','不替换'=>'1'))
							->set('tipe','替换相对地址，可以让在二级目录的7ghost正常运行，影响样式文件、脚本、站内链接')
							->e();
					w('select')->set('name','替换CSS相对地址')
							->set('key','relativeCSS')
							->set('value',isset($item['relativeCSS'])?$item['relativeCSS']:'0')
							->set('options',array('替换'=>'0','不替换'=>'1'))
							->set('tipe','替换相对地址，可以让在二级目录的7ghost正常运行，影响样式文件中的图片')
							->e();
					w('text')->set('name','模版文件名')
							->set('key','template')
							->set('value',isset($item['template'])?$item['template']:'')
