...
<?php include tpl('menu');?>
	<div class="main">
			<form action="" method="POST">
				<h2 class="section-header">自定义页<?php if(isset($page['name']) && $page['name']): ?>(<?php _e($page['name'])?>)<?php endif; ?>-编辑内容替换</h2>
				...
				<?php
					...
					w('text')->set('name','查找内容')
							->set('key','seach')
							->set('value',isset($item['seach'])?$item['seach']:'')
							->set('tipe','查找需要替换的内容,为正则匹配')
							->e();
					
					w('textarea')->set('name','替换为')
							->set('key','replace')
							->set('value',isset($item['replace'])?$item['replace']:'')
							->set('tipe','将查找到的内容替换为')
							->e();
				?>
