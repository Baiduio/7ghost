...
<?php include tpl('menu');?>
	<div class="main">
		<h2 class="section-header">自定义页面<?php if(isset($page['name']) && $page['name']): ?>(<?php _e($page['name'])?>)<?php endif; ?> - <a href="./?m=page&a=edit&key=<?php _e($_GET['key'])?>">基本</a>
			...
			<table class="ae-table ae-table-striped ae-quota-requests">
		        ...
		        <tbody>
		        	<?php
		        		...
		        		$key = $_GET['key'];
		        		$replaces = array();
		        		if(isset($pages[$key]) && isset($pages[$key]['replaces'])){
		        	...
		        	<tr <?php _e($i);?>>
		                <td>
		                	<div class="row-title">
		                    <a href="./?m=page&a=EditReplace&key=<?php _e($key)?>&rekey=<?php _e($rekey)?>" title="编辑<?php _e(isset($item['name'])?$item['name']:'')?>"><?php _e(isset($item['name'])?$item['name']:'')?></a>
		                	</div>
		                ...
		                <td>
			                   <?php _e(htmlspecialchars(isset($item['seach'])?$item['seach']:''))?>
		                </td>
		                <td>
			                    <?php _e(htmlspecialchars(isset($item['replace'])?$item['replace']:''))?>
			                </td>
		            </tr>
		        			<?php
		        		}
