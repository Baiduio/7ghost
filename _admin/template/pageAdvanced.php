...
<?php include tpl('menu');?>
	<div class="main">
		<h2 class="section-header">自定义页面<?php if(isset($page['name']) && $page['name']): ?>(<?php _e($page['name'])?>)<?php endif; ?> - <a href="./?m=page&a=edit&key=<?php _e($key)?>">基本</a>
				- 高级
		...
		<?php
			w('select')->set('name','COOKIES设置')
					->set('key','cookies')
					->set('value',isset($page['cookies'])?$page['cookies']:'0')
			...
			w('text')->set('name','自定义COOKIES')
					->set('key','diyCookies')
					->set('value',isset($page['diyCookies'])?$page['diyCookies']:'')
		...
		<?php
			w('select')->set('name','伪造agent')
					->set('key','agent')
					->set('value',isset($page['agent'])?$page['agent']:'0')
			...
			w('text')->set('name','自定义agent')
					->set('key','diyAgent')
					->set('value',isset($page['diyAgent'])?$page['diyAgent']:'')
		...
		<?php
			w('select')->set('name','伪造referer')
					->set('key','referer')
					->set('value',isset($page['referer'])?$page['referer']:'0')
					->set('options',array('自动伪造'=>'0','自定义referer'=>'1'))
					->set('tipe','自动伪造可以还原原始的referer,使全站镜像可以完美突破防盗链')
					->e();
			w('text')->set('name','自定义agent')
					->set('key','diyReferer')
					->set('value',isset($page['diyReferer'])?$page['diyReferer']:'')
